...
export class Footnotes {
	constructor(public notes: Record<string, ReactElement>) { }
	...
	footer(): ReactElement {
		return <div style={{ width: "100%", borderTop: "1px solid grey", paddingTop: "1ex" }}>{
			Object.entries(this.notes).map(([key, note], index) => <small style={{ display: "block" }} id={key} key={key}><a href={`#up-${key}`}>{index + 1}</a>: {note}</small>)
		}</div>
	}
}